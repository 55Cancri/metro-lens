...
const deleteItems = async (busStatusBatchOf25: PrimaryKey[]) => {
  const items = busStatusBatchOf25.map(({ entity, id }) => {
    // define the key
...
const run = async (lastKey?: PrimaryKey): Promise<any> => {
  ...
  const Params = {
    ...
    ExpressionAttributeValues: { ':pk': 'bus_status' },
    // ExpressionAttributeValues: { ':pk': 'route', ':skv': 'status_' },
  ...
  const statusOfBuses = Items as PrimaryKey[]

  // chunk the the payloads,
  const requests = chunk(statusOfBuses, 25)
