...
/**
 * The purpose of the auditor lambda in sequential order is to:
 * - Check if the stops and waypoints exist in dynamoDb and if not, load them from api calls.
 * - Query the status of buses and make api calls for buses that went offline over 10 minutes ago
 *
 *
 * @param event
 */
export const handler = async (event?: lambda.APIGatewayEvent) => {
  ...
  // TODO: query inside each prediction set
  const updatedVehicleStatusMap = reactivatedVehicles.reduce((store, vid) => {
    // TODO: iterate the active and dormant properties
    // TODO: iterate each prediction set within e.g. "1"

    return { ...store, [vid]: { isActive: true, wentOffline: null } }
  }, statusOfBuses)
